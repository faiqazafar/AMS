...
if ($filter === "all") {
    $result = mysqli_query($conn, "SELECT * FROM laptop ORDER BY lab_number, asset_tag");
} else {
    $result = mysqli_query($conn, "SELECT * FROM laptop WHERE lab_number = $filter ORDER BY asset_tag");
...
<head>
    ...
    <title>Laptop Specifications - ITAMS</title>
</head>
<body>
<div class="app-shell">
    ...
    <main class="main">
        <div class="topbar">
            <div>
                <div class="eyebrow">Category / Laptop</div>
                <h1>laptop Specifications</h1>
            </div>
            <a href="laptop.php" class="btn-main">+ Add Asset</a>
        </div>
            ...
            <?php if ($result && mysqli_num_rows($result) > 0) { ?>
                <table>
                    <tr>
                        ...
                        <th>Processor</th><th>RAM</th><th>Bluetooth</th><th>User_id</th><th>User_name</th>
                        <th>Action</th>
                    </tr>
                    <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars($row["bluetooth"]); ?></td>
                            <td><?php echo htmlspecialchars($row["user_id"]); ?></td>
                            <td><?php echo htmlspecialchars($row["user_name"]); ?></td>
                            <td><a href="Update_laptop.php?id=<?php echo $row["id"]; ?>"><button>Update</button></a></td>
                        </tr>
                    <?php } ?>
                </table>
            <?php } else { ?>
                <div class="empty-state">
                    ...
                    <p>No laptop recorded <?php echo $filter !== 'all' ? 'for Lab ' . htmlspecialchars($filter) : 'yet'; ?>.</p>
                    <a href="laptop.php" class="btn-main">+ Add the first one</a>
                </div>
            <?php } ?>
        </div>
    </main>
